...
# Save data to SQLite
def save_data(basurero_id, nivel_ultrasonico, peso_actual):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cursor.execute("""
        INSERT INTO basureros (basurero_id, nivel_ultrasonico, peso_actual, timestamp)
        VALUES (?, ?, ?, ?)
    """, (basurero_id, nivel_ultrasonico, peso_actual, current_time))
    conn.commit()
    conn.close()
...
#    return {"message": "Ubicación recibida", "latitude": location.latitude, "longitude": location.longitude}
    ...
    lat_destino = -1.676089  # Ejemplo de latitud (destino)
    lon_destino = -78.651728  # Ejemplo de longitud (destino)
